Shader "Unlit/HaloSurfaceShader"
{
    Properties
    {
        _Volume("Volume", 3D) = "" {}
    ...
    CGINCLUDE
        ...
        bool wasTouched = (currentValue > _Range) && tex.r < _HaloSize;
                            touched += wasTouched;
                            ...
                            float normal = dot(normalize(tex3D(_NormalMap, uv).rgb), normalize(ray.dir));

                            // Code should be added to allow this to work with the exior surface
                            bool inHaloRange = wasTouched && (touched < _StepsToEffect) && normal <= _MinRadialOfHalo && normal >= _MaxRadialOfHalo;
                            haloDistance = inHaloRange ? step_size + haloDistance : 0;
        
                            outputColor = inHaloRange ? haloIncludedOutputColor : outputColor;
                            haloIncludedOutputColor = inHaloRange
                            ...
                            voxelColor = map(index, currentDensity * inRange);
                            ...
                            outputColor = BlendColor(otherColor * power, outputColor);
                            haloIncludedOutputColor = BlendColor(voxelColor * power, haloIncludedOutputColor);
                           ...
                           wasInHaloRange = inHaloRange;
                            ...
                            if (haloIncludedOutputColor.a > _Threshold)
                            {
                                break;
                            }

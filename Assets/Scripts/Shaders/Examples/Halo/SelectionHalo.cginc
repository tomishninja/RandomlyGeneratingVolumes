...
fixed4 frag(v2f i) : SV_Target
{
	...
	for (steps = 0; steps < _MaxSteps && t < _MaxDistance; ++steps)
	{
		...
		bool inHaloRange = d > -_Tolerance && (d - _HaloSize) < 0 && dotp <= _MinRadialOfHalo && dotp >= _MaxRadialOfHalo && (_SelectionIndex < 0 || abs(_Importance[currentIndex] - _SelectionIndex) < 0.01);
